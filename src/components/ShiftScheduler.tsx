  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Lập Ca Làm Việc</h2>
          <p className="text-slate-500">Xếp lịch trực quan bằng cách kéo thả các loại ca từ danh mục vào ô tương ứng của nhân viên.</p>
        </div>
        <div className="flex items-center gap-2 border border-slate-200 rounded-xl p-1 bg-slate-100">
          <button
            onClick={() => setActiveTab('grid')}
            className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${
              activeTab === 'grid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
            }`}
          >
            Bản Phân Ca Tuần
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${
              activeTab === 'shifts' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
            }`}
          >
      ...
      {activeTab === 'grid' ? (
        <div className="grid gap-6 lg:grid-cols-4 items-start">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {/* Weekly Nav controls */}
            <div className="flex flex-wrap gap-3 items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-50 border border-indigo-200 px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 disabled:opacity-50 transition-all"
            ...
            <div className="flex items-center gap-2 rounded-xl bg-slate-50 border border-slate-150 px-4 py-2.5 text-xs text-slate-600">
                      ...
                      <tr className="border-b border-slate-200 bg-slate-50/55">
                        ...
                        {weekDates.map((date, idx) => {
                          ...
                          return (
                            <th 
                              key={idx} 
                              className={`px-2 py-3 text-center border-l border-slate-150 z-0 ${
                                isToday ? 'bg-indigo-50/30' : ''
                              }`}
                            >
                              <div className="text-xs font-bold text-slate-400 uppercase">{dayName}</div>
                              <div className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold mt-1 ${
                            ...
                            {weekDates.map((date, idx) => {
                              ...
                              return (
                                <td
                                  ...
                                  className={`p-2 border-l border-slate-100 text-center align-top min-h-[80px] cursor-pointer transition-all relative ${
                                    isOver ? 'bg-indigo-50 border-2 border-dashed border-indigo-400' : ''
                ...
                {shiftTypes.map((shift) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between rounded-xl border p-3 cursor-grab active:cursor-grabbing transition-all shadow-2xs hover:shadow-xs border-l-4 ${
                        shift.color === 'emerald' ? 'border-l-emerald-500' :
                  ...
                  {Object.keys(SHIFT_COLOR_MAP).map((colorName) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-7 w-7 rounded-full ${color.dot} border-2 transition-all ${
                          newShiftColor === colorName ? 'border-slate-950 ring-2 ring-indigo-100 scale-110 shadow-xs' : 'border-transparent'
                        }`}
                        title={colorName}
                      />
                    );
